...
- [ ] Queue operations processed in FIFO order (created_at ASC)
- [ ] Rate limiter checks: Google (10 req/sec), Microsoft (2000 req/10min)
- [ ] Delay enforced if within rate limit window
- [ ] Exponential backoff: 5s first retry, 15s second retry
- [ ] retry_count incremented on each failure
- [ ] status updated: pending → processing → completed/failed
- [ ] Failed operations (retry_count > 3) marked as 'failed'
- [ ] Completed operations older than 7 days deleted
- [ ] calendarSyncService inserts queue operation on 429 error
- [ ] Queue processing logs: operations processed, errors encountered
- [ ] Integration test: Trigger 429 → Operation queued → Processed by cron
